...
public final class RarityConfigManager {
	...
	public static boolean useMadokuLuck() { return settings.useMadokuLuck; }

	static RaritySettings settings(Tier tier) {
		return tier == null ? null : settings.rarities.get(tier);
	}
	...
	private static JsonObject readObject(JsonObject source, String key) {
		if (source == null || key == null || key.isBlank()) return new JsonObject();
